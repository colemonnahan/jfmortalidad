...
Type objective_function<Type>::operator() ()
{
  ...
  // Super simple fixed effect for each period
  p.setZero();
  phi.setZero();
  //  chi.setZero();
  vector<Type> p3(K);
  ...
  for(int i=0; i<I; i++){
    for(int t=0; t<K;t++){
      phi(i,t)=inv_logit(phi2(t));
      p(i,t)=inv_logit(p2(t));
    // quick priors for stability
      if(i==0){
	p3(t)=p(i,t);
	phi3(t)=phi(i,t);
	// nll-= dnorm(p3(t), Type(.5), Type(5), true);
	// nll-= dnorm(phi3(t), Type(.5), Type(5), true);
      }
    }
    phi(i,0) = Type(1);
  }
